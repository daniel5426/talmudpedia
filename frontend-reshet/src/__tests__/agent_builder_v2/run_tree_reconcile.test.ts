...
describe("run tree reconciliation", () => {
  it("lets tree status override stream-only divergence and keep terminal authority", () => {
    ...
    state = reconcileRuntimeTree(state, treePayload, staticNodes)
  ...
  it("does not let a stale tree revive child runs after a local root cancellation", () => {
    const streamEvents: AgentExecutionEvent[] = [
      {
        event: "orchestration.child_lifecycle",
        run_id: "run-root",
    ...
    let state = applyRuntimeEvents(createEmptyRuntimeGraphState(), streamEvents, staticNodes, staticEdges)
    const childAfterCancel = state.runtimeNodes.find((node) => node.id === "runtime-run:child-a")
    expect(childAfterCancel?.data.executionStatus).toBe("failed")

    state = reconcileRuntimeTree(state, staleTree, staticNodes)
    const childAfterStaleTree = state.runtimeNodes.find((node) => node.id === "runtime-run:child-a")
    expect(childAfterStaleTree?.data.executionStatus).toBe("failed")
  })
})
