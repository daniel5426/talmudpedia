...
- streamed artifact draft invalidation events trigger a dedicated draft snapshot refetch into local editor state
...
- `context_window` snapshots from prompt submission and run stream events are surfaced through the hook state
- live `context_window.updated` stream events update the context meter before terminal completion
- stale post-run session reloads do not overwrite a newer terminal context-window snapshot
