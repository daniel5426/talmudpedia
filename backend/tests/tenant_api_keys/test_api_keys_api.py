...
async def _create_key(db_session, *, tenant_id, created_by, name="Embed Key", scopes=None):
...
async def test_api_keys_create_list_and_revoke(client, db_session):
    ...
    list_resp = await client.get("/admin/security/api-keys", headers=headers)
    ...
    revoke_resp = await client.post(
        f"/admin/security/api-keys/{payload['api_key']['id']}/revoke",
        headers=headers,
    )
