...
- `rag.operators.schema` now excludes runtime-only fields from authoring-required catalog hints, removes stale `org_unit_id` from the canonical pipeline wrapper contract, requires explicit `pipeline_type`, and exposes union-shaped config like `web_crawler.start_urls` when the runtime accepts multiple input forms.
- RAG schema-contract coverage now locks the stricter create-time wrapper contract: `pipeline_type` is required in the shared pipeline instance contract, and operator filtering differs between ingestion and retrieval catalogs.
...
- Result: passed (`37 passed, 6 warnings`)
