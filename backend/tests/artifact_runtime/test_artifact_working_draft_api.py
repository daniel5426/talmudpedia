...
async def test_artifact_working_draft_endpoints_persist_unsaved_snapshot(client, db_session):
    ...
    try:
        ...
        assert initial_draft["draft_snapshot"]["python_dependencies"] == "requests>=2.0"
        ...
        assert persisted_draft["draft_snapshot"]["python_dependencies"] == "httpx>=0.27"
...
@pytest.mark.asyncio
async def test_artifact_working_draft_update_merges_artifact_and_draft_key_scope(client, db_session):
    tenant, user = await _seed_tenant_context(db_session)
    app.dependency_overrides[get_current_principal] = _override_principal(tenant.id, user)
    ...
    try:
        ...
        assert len(rows) == 1
        assert rows[0].artifact_id == uuid.UUID(artifact["id"])
        assert rows[0].draft_key == draft_key
        assert rows[0].working_draft_snapshot["description"] == "seeded from draft key"
    finally:
        app.dependency_overrides.pop(get_current_principal, None)
