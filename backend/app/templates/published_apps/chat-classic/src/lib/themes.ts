...
export const palettes: Palette[] = [
  ...
  },
  {
    id: 12,
    name: "Steel Blue",
    description: "Professional, cool-toned slate",
    light: {
      ...
      "--chat-background": "oklch(0.99 0.005 240)",
    },
    dark: {
      "--background": "oklch(0.18 0.02 240)",
      ...
      "--chat-background": "oklch(0.18 0.02 240)",
    },
  },
  {
    id: 13,
...
function buildSurfaceVariables(
  ...
  const background = tokens["--background"] || (mode === "light" ? "oklch(1 0 0)" : "oklch(0.15 0 0)");
  const primary = tokens["--primary"] || "oklch(0.3982 0.105065 247.1473)";
  const muted = tokens["--muted"] || background;
  const border = tokens["--border"] || (mode === "light" ? "oklch(0.89 0 0)" : "oklch(1 0 0 / 18%)");
  const ring = tokens["--ring"] || (mode === "light" ? "oklch(0.708 0 0)" : "oklch(0.556 0 0)");
  const sidebarMix = mode === "light" ? "4%" : "5%";

  return {
    "--sidebar": tokens["--sidebar"] || `color-mix(in oklch, ${primary} ${sidebarMix}, ${background})`,
    "--chat-background": tokens["--chat-background"] || background,
    "--gradient-from": tokens["--gradient-from"] || background,
    "--gradient-to": tokens["--gradient-to"] || `color-mix(in oklch, ${primary} 20%, ${background})`,
    "--sidebar-accent": tokens["--sidebar-accent"] || muted,
    "--sidebar-border": tokens["--sidebar-border"] || border,
    "--sidebar-ring": tokens["--sidebar-ring"] || ring,
  };
}
...
function normalizePalette(palette: Palette): Palette {
  return {
    ...
    light: {
      ...palette.light,
      ...buildSurfaceVariables(palette.light, "light"),
    },
    dark: {
      ...palette.dark,
      ...buildSurfaceVariables(palette.dark, "dark"),
    },
  };
}
...
export function applyPalette(paletteId: number) {
  ...
  const sourcePalette = palettes.find((candidate) => candidate.id === paletteId);
  const palette = sourcePalette ? normalizePalette(sourcePalette) : null;
  if (!palette) return;
...
export function initPalette() {
  ...
  if (paletteId && palettes.find((candidate) => candidate.id === paletteId)) {
