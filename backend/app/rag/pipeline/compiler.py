...
class PipelineCompiler:
    ...
    def compile(
        ...
        structural_errors = self._validate_structure(visual_pipeline)
        errors.extend(structural_errors)

        if errors:
            return CompilationResult(success=False, errors=errors)

        semantic_result = self._validate_semantics(visual_pipeline)
        ...
        compatibility_errors = self._validate_compatibility(visual_pipeline)
        errors.extend(compatibility_errors)

        if errors:
            return CompilationResult(success=False, errors=errors, warnings=warnings)

        ordered_steps = self._topological_sort(visual_pipeline)
        ...
        dag = self._build_dag(visual_pipeline, ordered_steps)

        config_snapshot = self._build_config_snapshot(visual_pipeline)

        from bson import ObjectId
        executable = ExecutablePipeline(
            visual_pipeline_id=visual_pipeline.id,
            version=visual_pipeline.version,
            tenant_id=visual_pipeline.tenant_id,
            dag=dag,
