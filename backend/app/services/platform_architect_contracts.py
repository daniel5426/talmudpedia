...
def build_architect_graph_definition(model_id: str, tool_ids: list[str] | None = None) -> dict:
    instructions = (
        ...
        "If the worker finished but you need further changes from the same worker, use architect-worker-respond to continue that worker conversation on the same binding/thread context. "
        "If the worker finished and no further changes are needed, the next required step is architect-worker-binding-persist-artifact. "
