...
## Overview
The Pipeline Input UI feature replaces the generic JSON input dialog for running pipelines with a dynamic, operator-aware form system. It automatically discovers the runtime parameters for a specific compiled pipeline and generates a user-friendly interface based on the operator's configuration schema.

## Core Features
- **Automatic Schema Discovery**: The system identifies start nodes in the execution graph and exposes all runtime-mutable inputs, even when the node already has saved config values.
...
- **Node Drawers**: Each start node renders as its own collapsible drawer labeled with the node name and input count.
...
1. **Input Schema Endpoint** (`GET /admin/pipelines/executable-pipelines/{exec_id}/input-schema`):
   ...
   - Returns step-scoped runtime fields for every start node, using saved config values as runtime defaults/fallbacks.
...
2. **`DynamicOperatorForm`**: Orchestrates the rendering of the discovered schema, grouping fields by their respective steps and rendering each step inside a collapsible drawer.
...
- **Saved Config Fallback**: Validation still accepts saved node config when a user leaves an exposed runtime field unchanged.
