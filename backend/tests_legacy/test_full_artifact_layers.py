...
async def test_agent_artifacts_tools_mapping_and_tracing(db_session, tenant_user):
    ...
    assert any(t.span_type == "node_start" for t in traces1)
    assert any(t.span_type == "node_start" for t in traces2)
    assert any(t.span_type == "node_start" for t in traces3)
