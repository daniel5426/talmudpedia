...
function buildAgentGraph(modelId: string, toolIds: string[]) {
  const instructions = [
    ...
    "Use the UI Blocks tool only when visuals materially improve comprehension.",
    "UI Blocks are appropriate for concentration, ranking, summary metrics, side-by-side comparisons, hedge coverage summaries, exception dashboards, and outlier rankings.",
    "UI Blocks are usually not appropriate for purely explanatory answers, narrative deal explanations, caveats-only answers, or answers where one sentence is clearer than a chart.",
    "Do not output any data as pure JSON. Use JSON only in the UI Blocks tool.",
    "When you use UI Blocks, call the tool at most once per answer and bundle all blocks in that one call.",
    "Pass strict JSON only. Never pass DSL, prose, shorthand, or extra keys.",
    "The UI Blocks input shape is exactly { title?: string, subtitle?: string, rows: [{ blocks: Block[] }] }.",
    "Use only these block kinds: kpi, pie, bar, compare, table, note.",
    "Every block must include kind, id, span, and title.",
    "Note blocks must include text.",
    "KPI blocks must include value as a string.",
    "Pie and bar blocks must include data as [{ label: string, value: number }].",
    "Compare blocks must include leftLabel, leftValue, rightLabel, rightValue, and optional delta. Compare numeric values must be numbers, not strings.",
    "Table blocks must include columns and rows. Every table row must have the same number of cells as columns.",
    "Use only the allowed keys for each block. Do not invent synonyms. Do not add extra keys.",
    "Block ids must be unique within the bundle.",
    "The total span of blocks in a row must be at most 12.",
    "For bank or currency concentration, prefer pie or bar charts.",
    "For recent activity summaries, prefer kpis and optionally a table.",
    "For deal vs market context, prefer compare and optionally a note.",
    "For hedge coverage, prefer kpis, bar, compare, and a table of upcoming exposures.",
    "For operational exceptions, prefer kpis, bar, and a ranked table.",
    "For pricing outliers, prefer kpis, bar, compare, and a ranked table.",
    "For caveats or scope notes, prefer normal assistant text. Use note blocks only when the visual adds value.",
    "If UI Blocks fails with code INVALID_UI_BLOCKS, read the returned hint, fix the JSON, and retry exactly once.",
    "If the second UI Blocks attempt fails, stop retrying and answer with text only.",
  ].join(" ");
